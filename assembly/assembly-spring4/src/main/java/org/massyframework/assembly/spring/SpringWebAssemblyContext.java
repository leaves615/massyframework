...
/**
 * 实现装配件上下文的SpringWeb容器
 *
 */
public class SpringWebAssemblyContext extends XmlWebApplicationContext 
	 ...
	 * 
	 */
	public SpringWebAssemblyContext() {
	}
	...
	private void inject(Map<DependencyServiceResource, Object> serviceMap,
		...
		for (Entry<DependencyServiceResource, Object> entry: serviceMap.entrySet()){
			...
			String cName = entry.getKey().getCName();
			...
			//使用组件编号注册Bean定义
			beanFactory.registerBeanDefinition(cName, beanDefinition);
		}
	}
	/**
	 * 执行加载Bean定义
	 ...
	 */
	@Override
	public Object getService(String cid) throws ServiceNotFoundException {
		try{
			return this.getBean(cid);
		}catch(BeansException e){
			throw new ServiceNotFoundException(cid, e);
		}
	}
	...
	public <S> S getService(String cName, Class<S> requiredType) throws ServiceNotFoundException {
		try{
			...
			throw new ServiceNotFoundException("service cannot found: cName=" + cName + ", requiredType=" + requiredType + ".", e);
		}
	}
